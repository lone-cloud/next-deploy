...
import { Builder } from 'lambda-at-edge';
import { OriginRequestDefaultHandlerManifest as BuildManifest } from 'lambda-at-edge/types';
import uploadAssetsToS3 from 's3-static-assets';
...
import type { BuildOptions, ServerlessComponentInputs, LambdaType, LambdaInput } from '../types';
...
class NextjsComponent extends Component {
  ...
  async deploy(
    ...
    if (domain && subdomain) {
      const domainComponent = await this.load('domain');
