...
describe('Serverless Trace', () => {
  ...
  it('copies api page dependencies to api lambda artifact', async () => {
    const nodeModulesPath = path.join(outputDir, API_LAMBDA_CODE_DIR, 'node_modules');
  ...
  it('copies ssr page dependencies to lambda artifact', async () => {
    const nodeModulesPath = path.join(outputDir, DEFAULT_LAMBDA_CODE_DIR, 'node_modules');

    const nodeModulesExists = await fse.pathExists(nodeModulesPath);
    expect(nodeModulesExists).toBe(true);

    const nodeModulesFiles = await fse.readdir(nodeModulesPath);
    expect(nodeModulesFiles).toEqual(expect.arrayContaining(['dependency-a', 'dependency-b']));

    const dependencyAPath = path.join(nodeModulesPath, 'dependency-a');
    const dependencyAFiles = await fse.readdir(dependencyAPath);
    expect(dependencyAFiles).toEqual(expect.arrayContaining(['index.js', 'sub-dependency.js']));
  ...
  it('does not copy any .next/ files into lambda artifact', async () => {
    const nodeModulesPath = path.join(outputDir, DEFAULT_LAMBDA_CODE_DIR, '.next');
